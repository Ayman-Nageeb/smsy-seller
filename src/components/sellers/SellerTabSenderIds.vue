...
<script>
import api from "../../api";
import SenderIdStatusView from "../sender_ids/SenderIdStatusView.vue";
import SetSellerSenderIds from "./SetSellerSenderIds.vue";
export default {
  components: {
    SenderIdStatusView,
    SetSellerSenderIds,
  ...
  methods: {
    async loadSellerSenderIds() {
      this.loading = true;
      try {
        const response = await api.get(`/sellers/${this.userName}/sender-ids`);
